function out = distance2target(solar,sky,ocean,path,object,lat,lon,h,lat0,lon0,h0,roll,pitch,yaw,azimuth,elevator,roll0,pitch0,yaw0,fov, Data)
    ...
    for i = 1:640
        if (abs(delta_az) < deg2rad(fov/2)+atan(15/distance0)) && (abs(delta_el) < deg2rad(3/4*fov/2)+atan(6/distance0))
            if (u0-delta_u0)< i-320-0.5 && i-320-0.5 < (u0+delta_u0) && (w0-delta_w0) < j-240-0.5 && j-240-0.5 <(w0+delta_w0)
                sum_radiance = zeros(1,100);
                idx = 0;
                for n = (-0.5):0.1:(0.5)
                    for m = (-0.5):0.1:(0.5)
                        ifov_radiance = surfaceCross (Data,solar,sky,ocean,path,object,lat,lon,h,roll,pitch,yaw,azimuth,elevator,fov,i+m-320-0.5,j+n-240-0.5);
                        idx = idx + 1;
                        sum_radiance(1,idx) = ifov_radiance;
                    end
                end 
...
if count_faces > 0
    ...
    solar_ifov = solar/pi * (1 - 0.1 *(1-(1- cos(beta))^5)) * transmiss * (deg2rad(ifov/10))^2;
    ocean_ifov = ocean/pi * 0.9 *(1 - 0.1 *(1-(1- cos(beta))^5)) * transmiss * (deg2rad(ifov/10))^2;
    sky_ifov = sky/pi * (1 - 0.1 *(1-(1- cos(beta))^5)) * transmiss * (deg2rad(ifov/10))^2;
    object_ifov = object/pi * 0.1 *(1-(1- cos(beta))^5) * transmiss * (deg2rad(ifov/10))^2;
    ...
    if distance > -1
        ...
        solar_ifov = solar/pi * (1 - e) * transmiss * (deg2rad(ifov/10))^2;
        ocean_ifov = ocean/pi * e  * transmiss * (deg2rad(ifov/10))^2;
        sky_ifov = sky/pi * (1 - e ) * transmiss * (deg2rad(ifov/10))^2;
        path_ifov = path/pi*(deg2rad(ifov/10))^2*(1-exp(-0.25*distance/1000));
        
        sum_radiance =  solar_ifov + ocean_ifov + sky_ifov + path_ifov;
    else
        sum_radiance =  sky/pi*(deg2rad(ifov/10))^2 + path/pi*(deg2rad(ifov/10))^2;
    end
end
